...
final_ridge <- glmnet(, , alpha = 0, lambda = )
...
lasso_model <- glmnet(x, y, alpha = , lambda = grid)
...
set.seed(1)
cv_out_lasso <- cv.glmnet(x_train, y_train, alpha = , lambda = grid)
...
lasso_train <- glmnet(x_train, y_train, alpha = , lambda = )

## Predict on the test set using the selected lambda
lasso_pred <- predict(lasso_train, s = best_lambda_lasso, newx = )

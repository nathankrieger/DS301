...
True. We could fit a model to be exactly the same (over fitting) which can effectively cover all points exactly, making the training MSE = 0 which is less than the irreducible error. It's important to note that while the training MSE is 0, the actual irreducable error has not changed, its just hidden by over fitting.
...
Squared bias: The error that comes with trying to estimate the function. As flexibity of a model increases the bias decreases.
...
Expected test MSE: This is how well the model performs on the test data. This MSE is much more important than the training MSE. The error decreases as the flexibility increases until it hits a point where it begins to over fit, after that happens the error increasing again.
...
k-fold validation is much more consistent compared to a basic validation set approach. k-fold CV is more stable because every observation is eventually used for both training and testing.

Also, in the basic approach, a large part of the data is never used to train the model. k-fold CV ensures that k-1/k of the data is used for training in each iteration, which typically leads to a more accurate and less biased estimate of the test error.

The disadvantage of k-fold CV compared to validation set is that it is more computationally expensive because the model is fitted k times.

### (ii)

k-fold CV is a computationally cheaper version of LOOCV. LOOCV is a k-fold CV with k = n. 

k-fold CV also has a lower variance in the error estimate compared to LOOCV because of the bias variance trade off. Since LOOCV training sets are nearly the same besides 1 entry, there is a higher variance when compared to k-folds with a reasonable k (say 5-20).


To go with this, a disadvantage of the k-folds approach compared to the LOOCV approach is that it has a higher bias. LOOCV uses all but 1 entries of the data for training so it is less bias because of the bias variance trade off. k-folds uses slightly less data causing it to be slightly more bias. 
...
LOOCV trains the model on n - 1 observations so it gives a better, more accurate estimate of the test error when comparing it to a standard test, training set approach. Each training set is nearly full sized.
...
As k increases, the variance increases. This is because as k increases, the size of the testing data decreases which increases variance. When the test fold size decreases, every prediction has a bigger effect on the estimate which increases variance. Averaging highly correlated values results in higher variance. 
